...
{
    public class OllamaAiRepository : IAiRepository
    {
        ...
        public OllamaAiRepository(HttpClient httpClient,
        ...
        public async Task<string> GenerateResponseAsync(string prompt, string conversationHistory)
        {
            try
            {
                ...
                var requestBody = new
                {
                    model = _modelName, // Now uses configured model name
                    prompt = prompt,
                    stream = false, // Explicitly set streaming preference
                    options = new
                    {
                        num_predict = 50,
                        num_threads = 2,
                        temperature = 0.67// Added for better control
                    }
                };
