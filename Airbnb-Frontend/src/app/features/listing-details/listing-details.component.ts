...
export class ListingDetailsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ratingCategories: RatingCategory[] = [];
   ...
   ngOnInit(): void {
      ...
      this.loading=true;
       this.subscription = this.listingsService.getListingById(listingId).subscribe({
        next: (data) => {
          this.listing=data;
...
updateVisibleAmenities(){
  ...
  this.visibleAmenities=this.showAllAmenities ? this.listing.amenities : this.listing.amenities.slice(0, this.defaultVisibleCount);
}

updateVisibleReviews(){
  this.visibleReviews=this.showAllReviews ? this.listing.reviews : this.listing.reviews.slice(0, this.defaultVisibleReviewsCount);
}
